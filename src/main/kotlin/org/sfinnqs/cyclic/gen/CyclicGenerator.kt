...
class CyclicGenerator(val config: WorldConfig) : ChunkGenerator() {
    ...
    override fun generateChunkData(
        ...
        if (!ChunkCoords(x, z).isRepresentative(config))
            return result
        ...
        for (localX in 0..15) {
            ...
            for (localZ in 0..15) {
                ...
                result.setRegion(
                    localX, 1, localZ,
                    localX + 1, height - 4, localZ + 1,
                    STONE
                )
                if (rawHeight < WATER_LEVEL + 1) {
                    result.setRegion(
                        localX, height - 4, localZ,
                        localX + 1, height, localZ + 1,
                        SAND
                    )
                    if (height < WATER_LEVEL)
                        result.setRegion(
                            localX, height, localZ,
                            localX + 1, 64, localZ + 1,
                            WATER
                        )
                } else {
                    result.setRegion(
                        localX, height - 4, localZ,
                        localX + 1, height - 1, localZ + 1,
                        DIRT
                    )
    ...
    override fun getDefaultPopulators(world: World): List<TreePopulator> =
        persistentListOf(TreePopulator(config))
